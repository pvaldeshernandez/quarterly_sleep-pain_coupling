...
At each quarterly visit, participants completed a set of self-report items assessing knee pain, body pain, and sleep quality. Two binary gateway items asked whether the participant had experienced any knee pain (q1) or any pain in the body (q6) in the past 7 days. These were followed by nine 0–10 numerical rating scale items (**Table 2**): four for knee pain intensity and stiffness (q2–q5), four for body pain intensity and stiffness (q7–q10), and one for sleep quality (q13). Two additional items assessed weekly fatigue (q11; 0 = no fatigue, 10 = as much fatigue as you can imagine) and mood (q12; 0 = very negative, 10 = very positive); these items were not included in the present analyses. When a participant reported no knee pain (q1 = 0) or no body pain (q6 = 0), missing intensity ratings for the corresponding region were set to zero. The gateway items themselves did not enter any statistical model.
...
For each ROI, mean contrast values were extracted from the first-level stimulation-versus-baseline contrast images (con\_0001.nii). For spherical ROIs, the unweighted mean was computed across all voxels within the sphere. For atlas-defined probabilistic ROIs, probability-weighted means were computed: $¯\_{i}=∑\_{v}BOLD\_{v}·w\_{v} / ∑\_{v}w\_{v}$, where $w\_{v}$ is the atlas probability at voxel $v$. Atlases were resampled to the fMRI resolution (3 mm isotropic) using linear interpolation prior to extraction. For each atlas-defined ROI, probability-weighted grey matter volume was also computed from modulated GM images (smwc1 output from SPM DARTEL normalization, 1.5 mm isotropic, $V\_{vox}=1.5^{3}=3.375$ mm$\^3$): $Vol\_{i}=∑\_{v}GM\_{v}·w\_{v}·V\_{vox}$, where $GM\_{v}$ is the modulated grey matter probability at voxel $v$. All five atlas-defined ROIs were viable at VBM resolution, including the PBN which had only 3 voxels at the coarser fMRI resolution.

Two sets of first-level contrast images were generated using different explicit masks: a grey matter mask (study-specific DARTEL template GM probability thresholded at $>$ 0.25) and a whole-brain mask (all voxels with nonzero signal). The grey matter mask causes variable voxel dropout for structures that span the grey-white boundary (e.g., thalamus, ACC), are too small and inferiorly located to survive masking at 3 mm resolution (e.g., PBN), or contain a mixture of tissue types (e.g., insula, basal forebrain). Each ROI was therefore assigned to the mask that provided the most complete coverage of its voxels. Whole-brain contrasts were used for S1, middle insula, thalamus, anterior insula, ACC, PBN, SI-BF/Ch4, CeA, and BNST; GM-masked contrasts were used for both NAcc ROIs and LH.

### Sleep-to-pain ROIs

Within the Krause et al. (2019) sleep deprivation framework, six spherical ROIs were defined at coordinates where pain-evoked activation changed following one night of total sleep deprivation (**Figure S4**) (23). The original Krause et al. (2019) regions comprised right S1, right middle insula, left thalamus, left anterior insula, and bilateral NAcc. Because stimulation in the present study was applied to the most painful knee (right in 125 participants, left in 96), hemisphere assignment for somatotopically organized regions was determined by the stimulation side rather than adopting the original right-hemisphere coordinates. Given the predominantly contralateral activation of S1 during nociceptive stimulation (23, 49), this ROI was extracted from the hemisphere contralateral to the stimulated knee (MNI ±36, −45, 59; radius = 8 mm; 82 voxels). The middle insula, which also exhibits contralateral bias (23), was similarly extracted from the contralateral hemisphere (MNI ±32, 4, 11; radius = 8 mm; 72 voxels).

The remaining ROIs were considered to reflect lateral specialization rather than stimulus-side dependence, and were therefore retained at the original Krause et al. (2019) coordinates. The thalamus was kept ipsilateral to the original Krause et al. (2019) coordinates (left; MNI -10, -6, 10; 4 mm; 10 voxels), consistent with the marginal lateralization of thalamic pain responses (~20% greater contralateral vs. ipsilateral activation, compared to ~110% for S1; (49)). The anterior insula was kept in the left hemisphere (MNI -27, 25, 0; 8 mm; 82 voxels), consistent with bilateral-to-left-lateralized insular pain responses (49). Both nuclei accumbens (NAcc; $\pm $9, 2, -7; 6 mm; 17 brain voxels) were retained as bilateral fixed-coordinate ROIs because they index dopamine-mediated pain inhibition rather than somatotopic nociceptive processing.

An additional spherical ROI was defined in the right dorsal ACC/mid-cingulate cortex (dACC/MCC) at the peak coordinate from a meta-analysis of 222 pain fMRI experiments (50) (MNI 6, 12, 38; radius = 6 mm; 32 brain voxels; **Figure S4**), consistent with the right lateralization of ACC pain responses (51). This ROI was motivated by Sardi et al. (2024), who showed that the ACC and NAcc function as parallel dopamine D2-gated nodes: D2 agonist infusion into either region prevents sleep-restriction hyperalgesia in rats (28).

### Pain-to-sleep ROIs

Within the Lynch et al. (2025) pain-arousal relay framework (31), five bilateral ROIs corresponding to nodes of the $PBN^{elCGRP}$ pain-arousal pathway were defined using published probabilistic atlases to maximize anatomical specificity for these small subcortical structures (**Figure S6**). The lateral parabrachial nucleus (PBN) was defined from the Brainstem Navigator atlas (52), which provides probabilistic parcellations of brainstem nuclei in MNI152 space at 1 mm isotropic resolution; labels 19 and 20 (left and right lateral PBN) were combined into a single bilateral mask (3 brain voxels at 3 mm fMRI resolution). The substantia innominata/basal forebrain Ch4 cell group (SI-BF/Ch4) was defined from the probabilistic cytoarchitectonic atlas of Zaborszky et al. (2008), which provides continuous probability maps in MNI152 space (53); the Ch4 probability map was thresholded at any probability > 0 and used as a weighted extraction mask. The central nucleus of the amygdala (CeA) was defined from the CIT168 high-resolution in vivo atlas (54); because the published atlas provides an extended amygdala volume (combining CeA and BNST), we constructed a CeA-specific probabilistic map by extracting the centromedial amygdala label (AMY\_CEN, label 4) from the crowd-sourced individual-observer labelings available in the atlas repository (2 observers $×$ 8 template brains = 16 labelings), averaging the binary masks into a probability map in CIT168 native space (0.7 mm isotropic), registering to MNI152 1 mm space via FSL FLIRT (12-parameter affine) followed by FNIRT (nonlinear) because the CIT168 atlas is defined in its own template space rather than MNI152, and mirroring across the midline to create a bilateral atlas. The bed nucleus of the stria terminalis (BNST) was defined from the probabilistic atlas of Theiss et al. (2017), derived from 3T MRI data in MNI152 space (55). The lateral hypothalamus (LH) was defined from the probabilistic hypothalamic atlas (56); labels 25 and 26 (left and right lateral hypothalamic area) were combined into a bilateral mask. All five ROIs were extracted bilaterally regardless of the stimulated knee, because these structures belong to the spino-parabrachio-amygdaloid and medial pain pathways, which receive predominantly bilateral projections and lack the somatotopic organization characteristic of the lateral pain system.

## Statistical model

### Within-person decomposition

All time-varying variables ($Y\in P,S,K$) were decomposed into between-person means ($\overbar{Y}\_{i}$) and within-person deviations ($Y\_{it}^{w}=Y\_{it}-\overbar{Y}\_{i}$). Only within-person components entered the model, removing all stable between-person confounds by construction (57), a critical step given that standard cross-lagged panel models conflate within- and between-person variance and can yield misleading estimates of temporal dynamics (19). Lagged within-person variables ($Y\_{i,t-1}^{w}$) were created within continuous observation segments. Interaction terms ($S×K$, $P×K$) were computed from lagged within-person components; because these components are person-mean centered by construction, their products are correctly centered without further adjustment. Observations were organized into maximal runs of consecutive quarters with both pain and sleep observed; only segments of three or more quarters were retained, providing at least two usable lagged transitions per segment.
...
Innovations were modeled as correlated using a Cholesky decomposition: $ε\_{p,it}∼N(0,σ\_{p}^{2})$ and $ε\_{s,it}|ε\_{p,it}∼N(ρσ\_{s}/σ\_{p}·ε\_{p,it},σ\_{s}^{2}(1-ρ^{2}))$, equivalent to bivariate normal innovations with correlation $ρ$. Weakly informative priors were placed on all parameters: fixed effects $∼N(0,5)$; moderation parameters $γ∼N(0,1)$; random effect SDs $τ\~HalfCauchy(1)$; innovation SDs $σ\~HalfCauchy(2)$; and innovation correlation $ρ\~Beta(2,2)$ rescaled to $[-1, 1]$.

Estimation used the No-U-Turn Sampler (NUTS) with 4 chains, 2,000 tuning iterations, and 2,000 posterior draws per chain (8,000 total). All models were implemented in PyMC 5.27.1 (58), a probabilistic programming library for Python that provides tools for constructing and fitting Bayesian models using intuitive syntax and supporting a wide range of statistical models with GPU acceleration via PyTensor backends, leveraging the GPU resources available on the University of Florida's HiPerGator high-performance computing cluster.

### Model comparison

To assess the evidence for each coupling direction, four nested models were fit and compared via approximate leave-one-out cross-validation (LOO-CV) using Pareto-smoothed importance sampling (PSIS) (59), as implemented in ArviZ (60): (1) the full model with both coupling directions; (2) pain-to-sleep coupling only ($λ\_{sp}=0$, no $u\_{sp}$); (3) sleep-to-pain coupling only ($λ\_{ps}=0$, no $u\_{ps}$); and (4) no coupling ($λ\_{sp}=λ\_{ps}=0$). All models retained coupled innovations, contrast terms, and age/sex moderation on whichever coupling directions were present. LOO-CV estimates how well each model would predict new, unseen observations by approximating the result of repeatedly refitting the model with one observation left out, without actually performing the computationally expensive refitting. The comparison metric is the expected log pointwise predictive density (elpd), which summarizes each model's out-of-sample predictive accuracy: higher elpd values indicate better prediction, and differences between models ($Δ$elpd) quantify the predictive contribution of each coupling pathway. The reliability of the LOO-CV approximation was assessed via Pareto $\hat{k}$ diagnostics, which flag individual observations where the importance sampling approximation may be inaccurate ($\hat{k}>0.7$).
...
Because sleep deprivation increased pain-evoked activation in S1, middle insula, thalamus, and anterior insula and decreased it in the NAcc (23), and because stronger coupling corresponds to a more negative coefficient (see Measures), higher activation in the first four regions and lower activation in the NAcc were expected to be associated with stronger sleep-to-pain coupling—yielding negative $γ\_{sp}$ for S1, middle insula, thalamus, and anterior insula, and positive $γ\_{sp}$ for both NAcc ROIs. For the ACC, Sardi et al. (2024) showed that the ACC and NAcc operate as parallel D2-gated nodes (28), so higher ACC activation during pain was likewise expected to index reduced D2 buffering capacity (positive $γ\_{sp}$, as for NAcc). Because the ACC derives from the Sardi et al. (2024) framework rather than the Krause et al. (2019) sleep deprivation findings, it is separate from the six-ROI sign concordance test. For the Lynch et al. (2025) arousal pathway ROIs, greater relay capacity (whether indexed by higher pain-evoked BOLD or larger grey matter volume) was expected to strengthen pain-to-sleep coupling (negative $γ\_{ps}$). Although these expected directions are specified for both pathways, whether they translate from acute or preclinical designs to trait-level moderation of longitudinal coupling in humans remains an open question. Two-sided Bayesian $p$-values ($p=2×min(P(γ<0),P(γ>0))$) are therefore reported for all moderation parameters.
...
For the within-person contrast moderator ($K^{w}$), $W$ ranges over the observed within-person centered pain localization values pooled across all persons and quarters. The JN boundary identifies a quarter-level threshold: it is the pain localization value at which the population-average coupling transitions from credibly nonzero to non-credible. Because pain localization varies within persons across quarters, the same individual may fall in the credible region in some quarters and not in others, depending on their pain pattern that quarter. For between-person moderators (i.e., $X\_{i}$), the JN boundary identifies a person-level threshold: individuals whose moderator value falls within the credible region exhibit coupling that is credibly different from zero.

In the classical frequentist framework, the JN boundary is obtained by solving a quadratic equation for the moderator value at which the $t$-statistic equals the critical value, yielding a closed-form solution that depends on the variance-covariance matrix of the estimated coefficients (61). In the Bayesian framework, no single standard error or test statistic exists; instead, the full joint posterior of $(λ\_{0},γ)$ determines the credible interval at each $W$. Because the joint posterior is not constrained to be Gaussian—it may be skewed, heavy-tailed, or otherwise non-normal—the frequentist quadratic solution does not apply, and hypothesis tests based on a $t$-distribution are not available. The grid-based approach used here is the Bayesian analogue: it accommodates any posterior shape and is computationally trivial. Specifically, 500 equally spaced grid points spanning the observed moderator range were evaluated across all 8,000 posterior draws. The JN boundary was then refined by linear interpolation between the two adjacent grid points where the credible interval bound crossed zero, yielding approximate sub-grid precision.
...
## Factor analysis and pain localization contrast

An exploratory factor analysis (principal axis factoring on the polychoric correlation matrix of eight pain items) was conducted to separate general pain severity from the knee-versus-body pain distribution. The number of factors to retain was evaluated using Horn's parallel analysis (44), which compares observed eigenvalues against those obtained from random data of the same dimensions (1,000 replications) to distinguish substantive factors from sampling noise. The first factor was dominant (eigenvalue = 5.56, 69.5% of variance) and exceeded the 95th-percentile random eigenvalue threshold (1.13). The second eigenvalue (0.89, 11.2% of variance) fell below the parallel analysis threshold (1.11), indicating that a one-factor solution would be sufficient on purely statistical grounds. Nevertheless, a two-factor solution was retained because the second factor constitutes the primary exogenous moderator in the VARX(1) model. The retained two-factor solution jointly accounted for 80.7% of the variance. The first factor (F1: General Pain) had all eight items loading positively (range: 0.78–0.86), capturing overall pain severity. The second factor (F2: Contrast) showed positive loadings on knee items (+0.30 to +0.45) and negative loadings on body items (-0.23 to -0.50), capturing the within-person contrast between knee-localized and body-wide pain. The two factors were orthogonal ($r=0.001$).

Because the second eigenvalue fell below the parallel analysis threshold, we provided external validation for the contrast factor. Each participant's mean contrast score across all available quarters ($\overbar{K}\_{i}$) was compared against baseline pain area endorsements and clinical measures not used in the factor analysis. Because the PHQ body map allows endorsement of multiple pain areas simultaneously (mean = 3.2 areas, SD = 2.5), participants were classified into three pain distribution groups: knee pain only (N = 19), knee pain plus at least one other area (N = 143), and no knee pain (N = 67). A one-way ANOVA revealed significant differences in $\overbar{K}\_{i}$ across groups ($F(2,226)=16.56$, $p<0.001$). Tukey post-hoc comparisons confirmed that all three groups differed from each other: knee-only participants had the highest average contrast scores ($\overbar{K}\_{i}=0.57$, SD = 0.64), followed by knee-plus-others ($\overbar{K}\_{i}=0.10$, SD = 0.72; $p=0.016$ vs knee-only), and no-knee participants had the lowest ($\overbar{K}\_{i}=-0.34$, SD = 0.61; $p<0.001$ vs knee-only and $p<0.001$ vs knee-plus-others). Complementarily, point-biserial correlations between each of the 13 individual pain area endorsements and $\overbar{K}\_{i}$ confirmed a clear pattern: knee endorsement was positively associated with the contrast ($r\_{pb}=0.31$, $p<0.001$), while all 12 non-knee areas showed negative or near-zero associations, with upper back ($r\_{pb}=-0.16$, $p=0.018$) and lower back ($r\_{pb}=-0.14$, $p=0.034$) showing the strongest negative effects; however, only knee endorsement survived FDR correction across the 13 tests (**Figure S1**).

Other baseline clinical measures further confirmed this pattern. The person-mean contrast correlated positively with all knee-specific continuous measures: PHQ knee pain days per week ($r=0.37$, $p<0.001$), PHQ percent of waking day in knee pain ($r=0.30$, $p<0.001$), WOMAC Pain ($r=0.28$, $p<0.001$), WOMAC Total ($r=0.26$, $p<0.001$), WOMAC Physical Function ($r=0.25$, $p<0.001$), WOMAC Stiffness ($r=0.24$, $p<0.001$), knee pain rating ($r=0.23$, $p<0.001$), and radiographic OA severity (Kellgren-Lawrence grade of the index knee; Spearman $ρ=0.31$, $p<0.001$). Scatter plots for all measures are shown in **Figure S2**. Taken together, these results suggest that the contrast factor captures meaningful variance in the relative predominance of knee-specific versus body-wide pain, distinct from overall pain severity.
...
Same-quarter innovations in pain and sleep quality were negatively correlated ($\hat{ρ}=-0.155$, $P(<0)=1.000$, 95% CrI [-0.202, -0.111]), indicating that within a given quarter, unexplained increases in pain co-occurred with unexplained decreases in sleep quality, or vice versa. Because this is a residual correlation after controlling for all lagged effects, it may reflect within-quarter bidirectional processes, shared contemporaneous perturbations (e.g., a flare event affecting both pain and sleep within the same assessment window), or measurement timing effects.
The model comparison (see Methods: Model comparison) confirmed the dominance of the pain-to-sleep pathway. Including pain-to-sleep coupling improved predictive accuracy substantially ($ΔLOO\_{elpd}$ = +22.0, SE = 8.8, $Δ$/SE = 2.49 for full vs. no-PS; $ΔLOO\_{elpd}$ = +23.1, SE = 9.1, $Δ$/SE = 2.53 for no-SP vs. null), whereas including sleep-to-pain coupling yielded negligible improvement ($ΔLOO\_{elpd}$ = +0.7, SE = 5.5, $Δ$/SE = 0.12 for full vs. no-SP; $ΔLOO\_{elpd}$ = +1.8, SE = 5.9, $Δ$/SE = 0.30 for no-PS vs. null). Pareto $\hat{k}$ diagnostics (59) confirmed the reliability of the LOO-CV approximation: only 2 of 1,818 observations (0.1%) exceeded the 0.7 threshold (maximum $\hat{k}$ = 0.90). By the conventional threshold of $|Δ/SE|>2$ (59), pain-to-sleep coupling substantially improved prediction while sleep-to-pain coupling did not.

## Moderation of pain location

The contrast moderation parameters ($\hat{δ}\_{p}$, $\hat{ω}\_{sp}$, $\hat{δ}\_{s}$, $\hat{ω}\_{ps}$) are reported in **Table 4**. The direct effect of pain localization on next-quarter sleep quality was credible ($\hat{δ}\_{s}=-0.050$, $P<0=0.986$, 95% CrI [-0.093, -0.006]): when a person's pain was more knee-dominant than usual, their next-quarter sleep quality worsened. The interaction between pain and localization trended toward strengthening the pain-to-sleep coupling when pain was knee-dominant ($\hat{ω}\_{ps}=-0.050$, $P<0=0.903$) but did not reach the 0.95 credibility threshold. Neither localization term affected the sleep-to-pain pathway.
...
## Moderation of sleep-pain coupling by fMRI response

For brevity, JN figures are presented only for moderators that reached or approached significance; JN analyses for the remaining Krause et al. (2019) ROIs (**Figure S5**) and for all arousal pathway ROIs (**Figures S7-S8**) are provided in the Supplementary Material.

Of the 229 participants in the coupling sample, 174 had usable task-based fMRI data from the baseline session and were included in the fMRI moderation analyses (115 female, 59 male; mean age = 58.7 years, SD = 8.5; 71 Black/African American, 47 White/Caucasian, 42 Hispanic/Latino, 14 other). Left NAcc activation during painful knee stimulation was the only Krause et al. (2019) ROI that significantly moderated sleep-to-pain coupling ($\hat{γ}\_{sp}=+0.040$, 95% CrI [+0.004, +0.076], $p=0.027$; **Table 5**): the lower the left NAcc activation during painful stimulation, the stronger the sleep-to-pain coupling. The right NAcc showed the same direction but did not reach significance ($\hat{γ}\_{sp}=+0.023$, 95% CrI [-0.013, +0.057], $p=0.194$). Johnson-Neyman analysis (**Figure S5b**) revealed that sleep-to-pain coupling was credibly negative for individuals with left NAcc activation below -0.027 (near the sample mean), encompassing 49% of the sample. Above this boundary, the coupling was not credibly different from zero.

Both ACC hemispheres—tested separately from the Krause et al. (2019) framework as Sardi et al. (2024)-motivated D2-gated nodes—significantly moderated sleep-to-pain coupling (**Table 5**): right dACC/MCC ($\hat{γ}\_{sp}=+0.038$, 95% CrI [+0.000, +0.077], $p=0.047$) and left dACC/MCC ($\hat{γ}\_{sp}=+0.042$, 95% CrI [+0.003, +0.081], $p=0.035$). Higher pain-evoked ACC activation was associated with less negative sleep-to-pain coupling in both hemispheres. Johnson-Neyman analyses (**Figure 5**) revealed patterns closely paralleling the NAcc, with credible sleep-to-pain coupling present only in individuals with below-average ACC activation in both hemispheres. The low correlation between ACC and left NAcc BOLD values ($r=0.16$) indicates that these regions capture largely independent aspects of pain processing, consistent with the view that they operate as parallel rather than serial nodes within the dopaminergic system (28).
...
**Figure 5.** Johnson-Neyman analyses of bilateral dACC/MCC BOLD moderation of sleep-to-pain coupling. Top panel: right dACC/MCC (MNI 6, 12, 38; 6 mm sphere; unmasked contrasts; $\hat{γ}\_{sp}=+0.038$, $p=0.047$). Bottom panel: left dACC/MCC (MNI -6, 12, 38; 6 mm sphere; unmasked contrasts; $\hat{γ}\_{sp}=+0.042$, $p=0.035$). In each panel, the blue line shows the posterior mean conditional coupling slope as a function of ACC activation (z-scored), dashed lines show the 95% credible interval, and green shading indicates the region where the CrI excludes zero. The dotted vertical line marks the JN boundary. Vertical markers show simple slopes at $z=-2$, $z=0$, and $z=+2$ with 95% CrI error bars. N = 174.

Although only the left NAcc and both ACC hemispheres reached individual significance among the eight sleep-to-pain ROIs (**Table 5**), all six Krause et al. (2019) univariate $\hat{γ}\_{sp}$ estimates matched the direction predicted by the sleep deprivation framework. The probability of this occurring by chance is $p=(1/2)^{6}=0.016$ (exact sign test), providing convergent support for the Krause et al. (2019) framework at the pattern level even though only the left NAcc reached individual significance.

## Pain-arousal relay pathway moderation

The five nodes of the Lynch et al. (2025) pain-arousal relay pathway were tested as moderators of pain-to-sleep coupling ($\hat{γ}\_{ps}$) using two complementary approaches: pain-evoked fMRI response (N = 174) and grey matter volume from atlas-defined probabilistic ROIs (N = 189). No individual ROI reached significance in either modality (**Table S1**)—a sensitivity analysis testing each hemisphere separately (10 models) confirmed that bilateral averaging did not mask lateralized effects. The BNST showed the strongest fMRI effect ($\hat{γ}\_{ps}=-0.077$, $p=0.166$), with the PBN—the origin of the pain-arousal relay pathway—showing the second strongest ($\hat{γ}\_{ps}=-0.075$, $p=0.145$). Notably, all five grey matter volume estimates were negative—consistent with larger arousal relay volumes predisposing stronger pain-to-sleep coupling—though no individual effect was credible (strongest: PBN, $\hat{γ}\_{ps}=-0.062$, $p=0.261$). Johnson-Neyman analyses for each ROI are shown in **Figures S7-S8**.
...
This finding suggests that, while day-to-day sleep fluctuations may have immediate consequences for next-day pain sensitivity—consistent sleep disruption impairing endogenous pain inhibition (20, 21) and increases pain sensitivity through peripheral and central mechanisms (22, 62)—at the quarterly timescale, accumulated pain burden may more strongly drive subsequent sleep quality through slower mediating processes such as inflammatory flare-and-remission cycles, changes in habitual sleep behavior, and seasonal shifts in activity.

A theoretical framework (63) supports the notion that quarterly coupling reflects processes distinct from—and potentially unrelated to—accumulated daily effects. Daily cross-lagged effects propagate only through autoregressive persistence; at the values typically reported (0.2–0.4) (7, 18), there is no path for daily sleep-to-pain effects to reach the next quarterly measurement. The negative innovation correlation ($\hat{ρ}$ = −0.155) suggests that some coupling also occurs within the quarter—for example, a pain flare disrupting sleep over ensuing weeks—but because these changes unfold between consecutive assessments, they appear as co-occurring residuals rather than lagged effects, potentially underestimating coupling strength relative to daily designs.
